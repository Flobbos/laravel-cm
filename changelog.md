...
- **Fatal fix (`Campaigns`, `Lists`)**: typed the redeclared `$skip_key` property as `?string` to match `BaseClient`. The 6.2.0 typed-property pass left these untyped, causing `Type of ...::$skip_key must be ?string (as in class BaseClient)` whenever a campaign or list service was instantiated.
- **Fatal fix (`Lists`, `Subscribers`)**: aligned the `ResultFormat` trait's `formatSubscribers()` signature with `ResultFormatContract` (`string $pageName`, `: LengthAwarePaginator`), which the 6.2.0 interface update had drifted from.
- **`SubscriberContract`**: removed a stale duplicate `formatSubscribers($result_body, $paginated = false)` declaration that conflicted with the canonical one on `ResultFormatContract`.
- **`Subscribers`**: fixed the PHP 8.4 implicit-nullable deprecation on `subscribe()` (`?string $name`).
- **Tests**: added a class-integrity test that loads every package class in isolation to catch incompatible property/method declarations going forward.
